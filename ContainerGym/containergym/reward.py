...
class RewardEvaluator:
    ...
    gaussian_reward_dict_params(action, current_volume, proc_unit_is_free, container_id)
        Calculates the reward for an action given the current container volume and whether the
        PU is free or not. Uses the parameters in container_params (dictionary form).

    gaussian_reward_list_params(action, current_volume, proc_unit_is_free, container_id=None)
        Calculates the reward for an action given the current container volume and whether the
        PU is free or not. Uses the parameters in container_params (list form).

    reward(action, current_volume, proc_unit_is_free, container_id)
        Calculates the reward for an action given the current container volume and whether the
        PU is free or not. Depending on the type of container_params, calls either the
    ...
    def gaussian_reward_dict_params(
        self, action, current_volume, proc_unit_is_free, container_id
    ):
        """
        Calculates the reward for an action given the current container volume and whether the
        PU is free or not. Uses the parameters in container_params (dictionary form).
        ...
        proc_unit_is_free : bool
            True if the PU is free (emptying is possible) at time t. False otherwise.
        container_id : int
        ...
        if action > 0:
            if current_volume == 0.0 or not proc_unit_is_free:
    ...
    def gaussian_reward_list_params(
        self, action, current_volume, proc_unit_is_free, container_id=None
    ):
        """
    ...
    def reward(self, action, current_volume, proc_unit_is_free, container_id):
        """
        ...
        return self.gaussian_reward_dict_params(
            action, current_volume, proc_unit_is_free, container_id
        )
